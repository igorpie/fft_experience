...
*/
        ...
        // импульс нарезается на 4 части, в начале каждой части - нули для перехлеста
        FftHelper.clearArrayPart(temp, 0, IMPULSE_OVERLAPPED_SIZE);
        FftHelper.replaceArrayPart(impulseArray, 0 * IMPULSE_PART_SIZE, temp, IMPULSE_PART_SIZE, IMPULSE_PART_SIZE); // 0[256] + append impulse[256]
        Fft2.Complex2[] s1 = Fft2.calcFft2(temp);

        FftHelper.replaceArrayPart(impulseArray, 1 * IMPULSE_PART_SIZE, temp, IMPULSE_PART_SIZE, IMPULSE_PART_SIZE);// 0[256] + append impulse[next 256]
        Fft2.Complex2[] s2 = Fft2.calcFft2(temp);

        FftHelper.replaceArrayPart(impulseArray, 2 * IMPULSE_PART_SIZE, temp, IMPULSE_PART_SIZE, IMPULSE_PART_SIZE);
        Fft2.Complex2[] s3 = Fft2.calcFft2(temp);

        FftHelper.replaceArrayPart(impulseArray, 3 * IMPULSE_PART_SIZE, temp, IMPULSE_PART_SIZE, IMPULSE_PART_SIZE);
